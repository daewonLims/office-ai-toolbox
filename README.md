...
**개조식 변환기는 성격상 텍스트 자체를 전송해야만 동작합니다.** 대신 앱은 이 텍스트를 서버에 저장하지 않으며, 처음 변환할 때 확인 다이얼로그로 "텍스트가 AI로 전송된다"는 사실을 명시적으로 안내합니다. 회사 기밀·개인정보가 포함된 텍스트는 조직의 보안 정책을 확인한 뒤 사용하세요.
